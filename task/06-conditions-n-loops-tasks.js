...
function isCreditCardNumber(ccn) {
    ...
    for (let i = 0; i < ccn.length; i++) {
        if (ccn.length % 2 !== i % 2) summ += ccn[i] * 1;
        else summ += ccn[i] * 2 + (ccn[i] > 4 ? -9 : 0);
    }
    return summ % 10 === 0;
}
...
function isBracketsBalanced(str) {
    ...
    for (var i of str) {
        ...
        else if (closeBrackets.indexOf(i) === openBrackets.indexOf(result[result.length - 1]))
        result.pop();
        else result.push(i);
    }
